...
Mexdebt %>%
  rename(year = X__1, debt = `debt/GDP`) %>%
  select(year, debt) %>%
...
WDI(country = c("EU", "ZJ"), indicator = c("NY.GDP.MKTP.KD.ZG"),
  ...
  rename(gdppc = `NY.GDP.MKTP.KD.ZG`)  %>%
...
```{r bra-rok-auto-export-quality, echo=F, eval=T, fig.width = 14, fig.height = 8.5, warning = F, message = F}
IMF <- readxl::read_xlsx("~/Dropbox/teaching/posc1020/development-2/Export_Quality_Index_7321_Passenger.xlsx", sheet=2) %>% slice(1:5)

IMF %>% slice(1:2) %>%
  gather(Year, Value, `1963`:`2014`) %>%
  rename(Country = X__1) %>%
  mutate(Country = ifelse(Country == "Brazil", "Brazil", "South Korea")) %>%
...
```{r bra-rok-auto-production-1950-2016, echo=F, eval=T, fig.width = 14, fig.height = 8.5, warning = F, message = F}

tribble(
  ~Country, ~Year, ~Value,
  "South Korea", 1950, 0,
  "South Korea", 1960, 0,
  "South Korea", 1970, 28819,
  "South Korea", 1980, 123135,
  "South Korea", 1990, 1321630,
  "South Korea", 1995, 2526400,
  "South Korea", 2000, 3114988,
  "South Korea", 2005, 3699350,
  "South Korea", 2010, 4271741,
  "South Korea", 2011, 4657094,
  "South Korea", 2012, 4561766,
  "South Korea", 2013, 4521429,
  "South Korea", 2014, 4524932,
  "South Korea", 2015, 4555957,
  "South Korea", 2016, 4228509,
  "Brazil", 1950, 0,
  "Brazil", 1960, 133041, 
  "Brazil", 1970, 416089,
  "Brazil", 1980, 1165714,
  ...
  mutate(Value = Value/1000,
         labs = prettyNum(round(Value), ",")) %>%
  ggplot(.,aes(as.factor(Year), Value, fill=Country)) + theme_steve_web() +
  geom_bar(stat="identity", position = "dodge", color="black") +
  scale_y_continuous(labels = scales::comma) +
  xlab("Year") + ylab("Motor Vehicle Production (in Thousands)") +
  geom_text(aes(label=labs, group=Country), color="black",
            position=position_dodge(width=.9), size=3, vjust = -.5, family="Open Sans") +
