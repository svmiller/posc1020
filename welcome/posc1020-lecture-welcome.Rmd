...
lastday <- as.Date("2019-04-26")
finalexam <- as.Date("2019-05-02") # 8 a.m.
...
Ms. Temitope Arogundade (tarogun@g.clemson.edu) will serve as a graduate teaching assistant this semester. 

- She will assist in proctoring exams and grading your short papers.
...
| Map Quiz | Jan. 31 | 10% | 
| Midterm 1         | Feb. 21 | 20% |
| Midterm 2 | April 11 | 20% |
| Short Paper | April 13 | 10% |
| Final | May 2, 8-10:30 a.m. | 30% |
...
remove_dates <- c(seq(as.Date("2019-03-18"), as.Date("2019-03-22"), by = "1 day"),
                  seq(as.Date("2019-03-25"), as.Date("2019-03-29"), by = "1 day"))
...
exam_dates <- c(as.Date("2019-01-31"), as.Date("2019-02-21"), as.Date("2019-04-11"), finalexam)
myfills <- ifelse(mydate %in% exam_dates, " Exam/Quiz " ,myfills)

# Fill in paper due dates

myfills <- ifelse(mydate %in% c(as.Date("2019-04-13")), " Paper Due (Turnitin) ", myfills)

give_me_days <- seq(as.Date("2019-01-01"), as.Date("2019-05-31"), by="1 day")
...
#  geom_point(shape=1, size=7) +
  ...
  labs(caption = "Notable dates: Jan. 31 (Map Quiz), Feb. 21 (Midterm 1), March 18-22 (Spring Break), March 25-29 (No Class), April 11 (Midterm 2), April 13 (Paper Due, Turnitin), May 2 (Final Exam)")
```
