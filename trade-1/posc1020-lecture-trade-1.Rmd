...
bind_rows(SugA, SugG) %>%
  ...
  scale_x_date(date_breaks = "2 years",
               date_labels = "%Y") +
  ylab("Price of Sugar (in USD, per Metric Ton)") + xlab("") +
...
PRD %>%
  ...
  labs(colour = "Category",
       ...
       caption = "Data: Correlates of War Dyadic Trade Data (v. 4.0) and Gibler-Miller-Little MID Data (v. 2.03)")
...
tribble(
  ...
  labs(caption = "Data: U.S. Census",
       ...
       subtitle = "Capital goods (e.g. commercial aircraft [$121 billion], industrial machines [$51 billion], semiconductors [$44 billion], and telecommunications [$41 billion]) lead the way among all export types.")
...
WDI(country = "US", indicator = "NE.TRD.GNFS.ZS",
    start = 1960, end = 2017, extra = FALSE, cache = NULL) %>%
  rename(trade = `NE.TRD.GNFS.ZS`) -> wditrade

wditrade %>%
  ggplot(.,aes(year,trade)) + theme_steve_web() +
  geom_line(size=1.1) +
  geom_ribbon(aes(ymin=0, ymax=trade),
              alpha=0.3, fill="#619CFF") +
  ylab("Trade as % of GDP") + xlab("Year") +
  scale_x_continuous(breaks=seq(1960, 2015, by=5)) + xlab("") +
  labs(caption = "Data: World Bank Development Indicators",
...
###
```{r usa-trade-gdp-1790-2016, echo=F, eval=T, fig.width = 14, fig.height = 8.5, warning = F, message = F}
read_table("~/Dropbox/data/trade.txt") %>%
  select(year_1:gdpb_1) %>%
  magrittr::set_colnames(c("year", "impo", "expo", "bala", "gdpb")) -> y1900

wditrade %>%
  filter(year > 2006) %>%
  rename(perc = trade) -> y2007

read_table("~/Dropbox/data/trade.txt") %>%
  select(year:gdpb) %>%
  bind_rows(.,y1900) %>%
  mutate(perc = ((impo+expo)/gdpb)*100) %>%
  bind_rows(.,y2007) %>%
  ggplot(.,aes(year,perc)) + geom_line(size = 1.1) + theme_steve_web() +
  geom_ribbon(aes(ymin=0, ymax=perc),
              alpha=0.3, fill="#619CFF") +
  scale_x_continuous(breaks=seq(1790, 2020, by = 10)) +
  xlab("") + ylab("Trade as % of GDP") +
  labs(title = "The Volume of Trade as Percentage of GDP in the United States, 1790-2016",
       ...
       caption = "Data: Various sources (see especially: http://www.econdataus.com/tradeall.html). Post-2006 data come from the World Bank.")
