...
WDI(country=c("GR", "US", "IE", "PT", "ES"), indicator = "SL.UEM.TOTL.ZS", start=2000, end=2017) %>%
...
WDI(country=c("XD", "XO"), indicator = "BX.KLT.DINV.CD.WD", start=1970, end=2017) %>%
...
Migra <- read.csv("~/Dropbox/teaching/posc1020/finance-1/usmigration2017.csv") %>%
  gather(Country, Count, 1:ncol(.))
  
Migra %>%
  arrange(-Count) %>%
  mutate(sum = sum(Count, na.rm=T),
         perc = Count/sum,
         narr = paste0(round(perc*100, 2),"%")) %>%
  head(10) %>%
  ggplot(.,aes(reorder(Country, -perc), perc)) + theme_steve_web() +
  geom_bar(stat="identity", alpha=0.8, color="black", fill="#619cff") +
  geom_text(aes(label=narr), vjust=-.5, colour="black",
            position=position_dodge(.9), size=4) +
  xlab("Country") + ylab("Migrant Population as Share of all Migrants of all Origins") +
  scale_y_continuous(labels=scales::percent) +
  scale_x_discrete(labels = c("Mexico", "Puerto Rico", "Philippines", "China",
                              "Canada", "Cuba", "Germany", "United Kingdom",
                              "Italy", "South Korea")) +
  labs(title = "U.S. Migration is Mostly Regional and Mexico Accounts for Just Under a Quarter of It",
       subtitle = "Mexico, Puerto Rico, Canada, and Cuba combine for about 33% of the U.S.' mid-year migrant population in 2017.",
...
Out <- read.csv("~/Dropbox/teaching/posc1020/finance-1/usmigration-out-2017.csv") %>%
  filter(year == 2017) %>%
  mutate(popn = as.numeric(gsub(",","",.$pop)),
         ccode = countrycode::countrycode(country, "country.name", "cown"))

Out %>%
  arrange(-popn) %>%
  select(country, pop, popn) %>%
  slice(-1:-9) %>%
  slice(-2:-4) %>%
  slice(-3:-5) %>%
  slice(-4:-5) %>%
  ...
  slice(-10) %>%
  head(11) %>%
  ggplot(.,aes(reorder(country, -popn), popn)) + theme_steve_web() +
  geom_bar(stat="identity", alpha=0.8, color="black", fill="#619cff") +
  geom_text(aes(label=pop), vjust=-.5, colour="black",
            position=position_dodge(.9), size=3.5) +
  scale_y_continuous(labels = scales::comma) +
  xlab("Country") + ylab("Size of Population Born in the U.S.") +
  labs(title = "Likewise: Most U.S. Migration is Local or to Familiar/Friendly Countries",
       subtitle = "Generally, U.S. migration follows to places where FDI is abundant.",
       caption = "Data: United Nations Population Division (DESA)")
