...
```{r colombia-civil-conflict-deaths-1989-2016, echo=F, eval=T, message=F, error=F, warning=F, fig.width = 14, fig.height = 8.5}

UCDP <- WDI(country="all", indicator=c("VC.BTL.DETH"), start=1946, end=2016) %>%
  rename(btldeath = VC.BTL.DETH)

UCDP %>%   filter(country == "Colombia") %>%
  filter(year > 1988) %>%
ggplot(., aes(year, btldeath)) +  theme_steve_web() +
  geom_hline(yintercept=1000, color="red", linetype = "dashed", size=1.5, alpha=0.3) +
  ...
  geom_text(aes(label=btldeath), color="black",
            ...
            size=4, vjust = -.3, family="Open Sans") +
